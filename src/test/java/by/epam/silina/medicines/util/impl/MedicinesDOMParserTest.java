...
class MedicinesDOMParserTest {
    private final MedicinesDOMParser medicinesDOMParser = MedicinesDOMParser.getInstance();
    ...
    @Test
    void parse() throws DOMParserException {
        List<Medicine> actualMedicines = medicinesDOMParser.getMedicines();
        Assertions.assertNull(actualMedicines);

        File file = new File(MEDICINES_XML_PATH);
        medicinesDOMParser.parse(file);

        actualMedicines = medicinesDOMParser.getMedicines();
